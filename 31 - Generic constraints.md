...
The final thing we wanted to do was make it impossible to put things like `Vegetable`s inside of our `FruitBasket`. We can do that by applying a _type constraint_. This tells TypeScript that our generic can't just be _any_ type. It has to match the constraints we supply. To do this, we'll use the `extends` keyword in our generic definition.
