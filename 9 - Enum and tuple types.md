...
We can see that our naïve solution didn't hold up well. Instead of giving us an array where the first item is a `string` and the second item is a function, `simpleUseState` returned an array where _each item_ is _either_ a `string` or a function. These are called Union types, and they are annotated with the vertical bar (`|`) between the different types. We'll learn more about them later on in the course.
