...
Still, Assertion signatures are safer than most methods, since the type checker will verify that the type we are asserting is at least _similar_ to the original type. That keeps us from asserting that one type is a totally incompatible type.
