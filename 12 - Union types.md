...
This is where Union types come in. Union types represent values that could be one of any number of types. We've already seen them a couple of times in the course, such as when we used optional chaining to get a value that was either a `string` _or_ `undefined`.
...
This reads like "The Coordinate type is either the CoordinateInterface type _or_ the CoordinateTuple type".
