...
Interfaces allow us to give names to the shape of objects. Objects aren't the only types we encounter in our program, though. For example, we might use the same Tuple type in multiple places, which means redefining it several times over. In this first example, we use a tuple of three `number`s to represent a point in 3D space.
