...
Remember, though, that our class has some static properties which should be present on the constructor function. For that, we have to create a _separate_ type that includes a [callable interface](https://fireship.dev/c/typescript/typescript-functions.md) for the constructor.
...
If we wanted to make this class construct signature work with more than just `Vegetable` classes, we could turn it into a [generic interface](https://fireship.dev/c/typescript/typescript-generics.md), where the generic type is the class instance.
