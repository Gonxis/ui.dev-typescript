...
TypeScript is very loose with how it interprets our literal types. For example, when we create an object literal, TypeScript infers that the types of the values are primitives, like `string` and `number`, not _literal types_, like `"hello"` and `"5"`.
