...
How is this even helpful? It might give us some type guarantees, but it also means we can't do anything with our values. Fortunately, we can convince TypeScript that an `unknown` or `any` value actually has a more specific type by using a process called type narrowing. This involves doing runtime checks which either prove that a value is a specific type or prove that it is _not_ a specific type.
