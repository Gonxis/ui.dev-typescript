...
This shows a very important behavior which we need to remember as we create our type definitions. We already know that classes represent both values and types. We can also merge namespaces with values and types. In this case, by defining and function and a namespace with the same name, we've created something that is both a namespace and a value. We go through the `eatFruit` _namespace_ to access the `FruitBasket` interface in our `eatFruit` _function_ definition. We won't go into much more detail about this behavior in this course, but you can read about it in the [TypeScript documentation](https://www.typescriptlang.org/docs/handbook/declaration-files/deep-dive.html).
